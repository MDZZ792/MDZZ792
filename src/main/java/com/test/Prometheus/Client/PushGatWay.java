...
public class PushGatWay {
    ...
    public static void Gauge() throws Exception {
        //因为同时建了两个表,所以需要CollectorRegistry,只建立一个表时可以省略.
...
//        Gauge.Timer durationTimer = null;
        ...
        try {
            ...
            gauge.labels("T_One").inc(100);
            //字段1的值减26
            gauge.labels("T_One").dec(26);
            gauge.labels("T_One").dec(10);
            gauge.labels("T_One_01").dec(300);
            gauge.labels("T_One_01").inc(500);
//            durationTimer = gauge.startTimer();
            // This is only added to the registry after success,
            // so that a previous success in the Pushgateway isn't overwritten on failure.
            Gauge lastSuccess = Gauge.build()
                    .name("my_batch2_job_last_success").help("Last time my batch job succeeded, in unixtime.").labelNames("Two").register(registry);
//            lastSuccess.setToCurrentTime();
            lastSuccess.labels("T_Two").dec(500);
            lastSuccess.labels("T_Two").inc(100);
        } finally {
...
//        map.put("a","2");
    ...
    public static void Summary() throws InterruptedException, IOException {
       ...
       for(int i=0;i<10;i++){
           Thread.sleep(200);
           summary.labels("TEST","TESTT","TESTTT").observe(new Random().nextInt(100));
       }
   ...
   public static void Counter() throws IOException {
       ...
       Map<String, String> map = new HashMap<String, String>();
       counter.labels("T_testC01").inc();
       counter.labels("T_testC02").inc(10);
...
//            System.out.println("前:"+metric);
            ...
            bean.setOne_01(JB.getString("One_01"));
...
//            Delete();
            ...
            //解决时间为科学计数法
            BigDecimal bd = new BigDecimal("1.53898048E9");
